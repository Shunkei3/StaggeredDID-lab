...
+ I want to create the case where CS's DID (with never-treated group) causes bias in the estimation of ATT(g,t) if we do not control for covariates.
...
sim2_did_out_naive_dt <- 
  tidy(sim2_did_out_naive) %>%
  data.table() %>%
  setnames("estimate", "value") %>%
  .[, type := "estimate"] %>%
  .[, pre_post := ifelse(time < group, "pre", "post")] %>%
...
plot_att_gt(sim2_did_out_naive_dt)
```
